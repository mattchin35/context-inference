...
def _(mo):
    mo.md(r"""
    [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/mazzulab/ssm/blob/master/notebooks/2c%20Input-driven%20linear%20model%20(LM-HMM).ipynb)

    **LM-HMM with input-driven Gaussian emissions**

    This marimo notebook is a conversion of `2c Input-driven linear model (LM-HMM).ipynb`.
    It demonstrates LM-HMM simulation, MLE/MAP fitting, and model selection with cross-validation.

    The original notebook used Colab shell magics (`!pip`, `!git clone`).
    Install dependencies in your environment before running:

    ```bash
    pip install -U matplotlib seaborn joblib scikit-learn scipy
    pip install git+https://github.com/mazzulab/ssm.git
    ```
    """)
    return
