...
def generate_daily_raster(specific_subsets): 
    ...
    def save_plot():
        ...
        if fig is not None:
            fig.savefig('raster_plot.svg', format='svg')
...
for main_key in results.keys():
    ...
    for output in output_list:
        if output in sub_dict:
            ...
            ax.plot(bin_centers, count_values, 'k-', linewidth=1, label=output)
            ...
            for transition in transition_list:
                ...
                patch = mpatches.Patch(color=color, alpha=0.075, label=transition)
                patches.append(patch)

            ax.legend(handles=patches)

            plt.title(f"{main_key} - {output}")
            plt.xlabel('Time(seconds)')
            plt.ylabel('Count')
            if save_plots == 'yes':
                plt.savefig(f"{main_key}_{output}.svg", format='svg')
            plt.show()
