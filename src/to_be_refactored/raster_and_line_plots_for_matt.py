...
def generate_daily_raster(name_of_cleaned_csv_file): #start_time_sec and end_time_sec are for subsetting the raster plot
    ...
    def new_plot(): #creates a new raster plot for each press of the create_new_plot option in the GUI
        ...
        fig.set_figheight(8)
        fig.set_figwidth(16)
        ax.eventplot(array_of_timestamp_arrays, linelengths = 0.5, linewidths=0.2,color='black') 
        ax.set_title('Behavioral Raster Plot')
...
#     Context_changes.pop() #needs to pop the final ContextC since the session ends there
    ...
    fig = plt.figure(figsize=(18,8))
    plt.plot(range(start_block,end_block), reward_left_values[start_block:end_block], color='orchid',linewidth=2.5) #end_block is the last context selected (eg, 4-->ContextC2)
    plt.plot(range(start_block,end_block), reward_right_values[start_block:end_block], color="cyan",linewidth=2.5) #start_block and end_block are parameters;
    ...
    fig = plt.figure(figsize=(18,8))
    plt.plot(range(start_block,end_block), lick_left_values[start_block:end_block], color="orchid",linewidth=2.5)
    plt.plot(range(start_block,end_block), lick_right_values[start_block:end_block], color="cyan",linewidth=2.5)
