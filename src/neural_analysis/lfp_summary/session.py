...
class LFPSummarySessionRequest:
    ...
    ``population_id`` is the selected probe ID. ``output_directory`` is a
            ...
            sites.append(
                LFPSiteConfig(
                    ...
                    lfp_path=_required_path(
                        probe.lfp_file,
                        f"probes[{probe.probe_id}].lfp_file",
                        "file",
                    ),
...
def _build_population(
    ...
    if population_id is None:
        return None
    probe = next(
        (item for item in session.probes if item.probe_id == population_id),
        None,
    )
    if probe is None:
        raise ValueError(f"unknown population ID: {population_id}")
    return UnitPopulationConfig(
        label=probe.display_label,
        probe_label=probe.probe_id,
        ...
        selected_channels=probe.unit_channels or (),
        quality_settings=(),
        stable_unit_ids=(),
...
def build_metadata_spike_phase_config(
    ...
    probe_id : str
        Stable probe identity used as the selected population identity.
    ...
    """
    if not any(probe.probe_id == probe_id for probe in session.probes):
...
def build_lfp_summary_config(
    ...
    elif session.cache_directory is not None:
        output_directory = session.cache_directory
    else:
