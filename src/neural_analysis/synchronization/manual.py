...
from src.neural_analysis.synchronization.alignment import save_stream_sync_npz
from src.neural_analysis.synchronization.spikeglx import read_digital_lines
...
def decode_flipper_barcodes(flipper_signal: np.ndarray, sample_rate: float=25000, barcode_present=True, plot=False):
    """Decode flipper transitions and optional 32-bit boundary barcodes.

    Args:
        flipper_signal: One-dimensional digital flipper signal on the
            acquisition-sample axis. Values are interpreted as Boolean.
        sample_rate: Acquisition sampling rate in Hz.
        barcode_present: Whether the signal contains the two expected 32-bit
            boundary barcodes.
        plot: Whether to display the existing diagnostic time trace.

    Returns:
        A three-item tuple containing the event-array dictionary, the usable
        flipper bounds in seconds, and either the decoded integer/bit-string
        barcode pair or ``None`` when ``barcode_present`` is false. Event
        indices are samples and event times are seconds.

    Raises:
        AssertionError: If barcode wrappers are requested but not detected.
    """
...
def main():
    """Run the existing hardcoded manual synchronization example.

    The function accepts no inputs. It reads the configured behavior pickle,
    flipper CSV, and acquisition files; writes the existing alignment note and
    optional spike-sync NPZ; and returns a dictionary of loaded metadata,
    decoded barcode/bounds information, timebase mappers, optional spike
    results, and left/right lick UTC tables. Times are seconds, sample indices
    are acquisition samples, and missing optional IMEC results are ``None``.
    """
