...
def make_residual_summary(
    ...
    for model_type, alpha in (("lasso", 1.0), ("elastic_net", 0.5)):
        for lambda_choice in ("lambda.min", "lambda.1se"):
            interaction_term = 0.4 if model_formula == "rewards_x_side" else 0.0
            rows.append(
                {
...
def write_session_residual_summaries(processed_path: Path, session_id: str, mouse: str, date: str) -> None:
    """Write per-session formula-specific residual-model summary CSVs."""
    processed_path.mkdir(parents=True, exist_ok=True)
    for model_formula in ("rewards_x_side", "rewards_plus_side"):
        make_residual_summary(
            ...
            processed_path / f"{session_id}_block_residual_model_summary_{model_formula}.csv",
            index=False,
        )
...
def make_saved_session(tmp_path: Path, mouse: str, date: str, timestamp: str) -> SimpleNamespace:
    ...
    write_session_residual_summaries(processed_path, session_id, mouse, date)
...
def test_collect_mouse_block_residual_model_summaries_writes_formula_csv(tmp_path):
    """Mouse-level collection should combine one formula-specific summary per session."""
    ...
    assert (collected["model_formula"] == "rewards_plus_side").all()
...
def test_load_cross_mouse_block_residual_model_summaries_combines_mice(tmp_path):
    ...
    for mouse in ("CT024", "CT025"):
        ...
        (
            ...
            .to_csv(
                cross_session_path / f"{mouse}_block_residual_model_summary_rewards_x_side.csv",
                index=False,
            )
        )
...
def test_collect_cross_mouse_block_residual_model_summaries_writes_combined_csv(tmp_path):
    ...
    make_residual_summary(
        ...
        cross_session_path / "CT024_block_residual_model_summary_rewards_plus_side.csv",
        index=False,
    )
...
def test_main_cross_mouse_metrics_collects_residual_model_summaries(monkeypatch):
    ...
    assert calls[-1][1]["model_formulas"] == ("rewards_x_side", "rewards_plus_side")
