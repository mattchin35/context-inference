...
from src.neural_analysis.lfp_summary_ct026_profile_locks import (
    acquire_ct026_profile_run_lock,
    recover_ct026_ppc_executor_locks,
)
...
def test_ct026_adapter_forwards_exact_resume_and_never_constructs_preview_artifacts(
    ...
    dependencies = CT026PPCProfileAdapterDependencies(
        ...
        run_isolated_profile_job=lambda **_: {"total_elapsed_seconds": 1.0},
        run_profile=generic_runner,
        config_fingerprint=lambda _: "config-id",
        source_fingerprint=lambda _: "source-id",
...
def test_ct026_profile_run_lock_preserves_live_foreign_and_mismatched_locks(
    ...
    with acquire_ct026_profile_run_lock(
        ...
        identity,
        hostname="host-a",
        process_exists=lambda pid: pid == 42,
    ):
        metadata = json.loads((run_directory / ".ct026-profile.lock").read_text(encoding="utf-8"))
        assert metadata == {"hostname": "host-a", "identity": identity, "pid": 42}

    lock_path = run_directory / ".ct026-profile.lock"
    lock_path.write_text(
        json.dumps({"hostname": "host-a", "identity": identity, "pid": 42}),
        encoding="ascii",
    )
    with pytest.raises(RuntimeError, match="live"):
        acquire_ct026_profile_run_lock(
            run_directory, identity, hostname="host-a", process_exists=lambda _pid: True,
        )
    assert lock_path.is_file()

    lock_path.write_text(
        json.dumps({"hostname": "host-b", "identity": identity, "pid": 42}),
        encoding="ascii",
    )
    with pytest.raises(RuntimeError, match="foreign"):
        acquire_ct026_profile_run_lock(
            run_directory, identity, hostname="host-a", process_exists=lambda _pid: False,
        )
    assert lock_path.is_file()

    mismatch = {**identity, "git_fingerprint": "other-git"}
    lock_path.write_text(
        json.dumps({"hostname": "host-a", "identity": mismatch, "pid": 42}),
        encoding="ascii",
    )
...
def test_ct026_executor_lock_recovery_is_explicit_and_exact(
    tmp_path: Path,
) -> None:
    """Child-death recovery never blindly removes PPC executor/writer locks."""
    identity = {
        "config_fingerprint": "config-id",
        "source_fingerprint": "source-id",
        "git_fingerprint": "git-id",
    ...
    lock_path = work_root / ".ppc-executor.lock"
    lock_path.write_text(
        json.dumps({"hostname": "host-a", "identity": identity, "pid": 42}),
        encoding="ascii",
    )
    with pytest.raises(RuntimeError, match="live"):
        recover_ct026_ppc_executor_locks(
            work_root, identity, hostname="host-a", process_exists=lambda _pid: True,
        )
    assert lock_path.is_file()
    ...
    with pytest.raises(RuntimeError, match="foreign"):
        recover_ct026_ppc_executor_locks(
            work_root, identity, hostname="host-a", process_exists=lambda _pid: False,
        )
    assert lock_path.is_file()
    ...
    recovered = recover_ct026_ppc_executor_locks(
        work_root, identity, hostname="host-a", process_exists=lambda _pid: False,
    )
