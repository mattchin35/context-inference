...
class RnnMDP(BaseMDP):
    ...
    def __init__(self, task_params: task_config.TaskParams, rnn_params: rnn_config.AgentConfig):
        ...
        self.go_cue = False
        self.stimulus_on = False
        self.len_ITI = 1
        self.ITI_counter = 0
        self.cue_counter = 0

        # self.output_vector = np.zeros(2)
        self.choice_timesteps = []
        self.loss_timesteps = []  # deprecated, here for backwards compatibility
    ...
    def initialize_task_state(self):
        ...
        self.go_cue = True
        self.set_input_vector(reward=0, prev_state=None)
    ...
    def set_input_vector(self, reward: float, prev_state: str):
        prev_input_vector = self.input_vector
        ...
        elif self.cur_state == prev_state and self.cue_counter < self.task_params.t_cue:
            stimulus_vector = prev_input_vector[2:]
            self.cue_counter += 1
        ...
        self.input_vector = np.concatenate([np.array([reward, int(self.go_cue)]), stimulus_vector])  # reward, go_cue, right_cue, left_cue
    ...
    def is_action_correct(self, action: int) -> bool:
    ...
    def simple_action_reward(self, correct: bool) -> float:
        """Simplest reward pattern: if correct at go cue, give reward. Else get incorrect reward value."""
        if self.go_cue:
            if rng.random() < self.params.active_reward_probability and correct:
                reward = self.params.mean_correct_reward
            else:
                reward = self.params.mean_incorrect_reward
        else:
    ...
    def step(self, action: int) -> Tuple[float, float]:
        ...
        reward = self.simple_action_reward(correct)
        ...
        if self.go_cue:
            self.choice_timesteps.append(self.cur_trial)
        ...
        self.set_input_vector(reward, prev_state)
        # self.set_output_vector()
...
def main():
    ...
    mdp = RnnMDP(task_params=task_params, rnn_params=rnn_params)
    actions = []
    for _ in range(100):
        # ic('\n')
        if mdp.cur_state == 'right':
            action = 0
