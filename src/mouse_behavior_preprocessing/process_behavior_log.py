...
def process_sessions():
    """Prepare raw behavior data files for analysis. This needs to be updated for 2026 refactoring"""
    ...
    # sess_id_full = mouse + '_' + date + '_' + sess_timestamp

    session_folder = raw_data_path / sess_id_full
    session_log = '{}.log'.format(sess_id_full)
    session_info_path = '{}_session_info.pkl'.format(sess_id_full)

    output_path = processed_data_path / mouse / sess_id_full
    ...
    for m, d, t in zip(mice, dates, timestamps):
        ...
        output_path = processed_data_path / m / sess_id_full
        if not output_path.exists():
            output_path.mkdir()
