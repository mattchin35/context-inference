...
**Current status:** U1-U4 are implemented. U4 awaits explicit usability
acceptance. The organization refactor described here is planned but not
authorized to begin. R0 inventory work requires separate approval; R1 source
movement additionally requires U4 acceptance and approval of its exact work
package.
...
| --- | --- | --- |
| U1 - session metadata | Implemented | Version-1 metadata loads, validates, resolves contained paths, and reports action availability for the supported Open Ephys and SpikeGLX layouts. |
| U2 - metadata-driven webapp | Implemented | The existing webapp accepts one metadata path and derives its existing session, probe, site, population, source, and cache controls. |
...
| U4 - guide and examples | Implemented; acceptance pending | The quickstart and three example metadata files document metadata creation, validation, webapp launch, local/Slurm handoff, resume, recovery, and rerender. |

The implementation sequence is recorded by the tests-first and implementation
commits from `75397e9` through `5e131fa`. This plan does not reopen those
interfaces except to move their implementations without changing behavior.

### U4 acceptance gate

Before R1 source movement, the user must explicitly accept the U4 workflow.
Acceptance means the quickstart is sufficient for the current supported path;
it does not authorize a real run, external copy, Slurm submission, or refactor
deletion.
...
the inventory document is reviewed, and the first R1 edit/caller/test list is
approved.
...
- metadata-derived configurations are equivalent; and
...
- `webapp/session_inputs.py`: metadata-derived and preserved legacy inputs;
...
- hardware identities and anatomical labels remain distinct;
...
2. Review the identical region-name normalization currently present in
...
3. Consolidate overlapping sorter, aligned-spike, channel-quality, and unit-
   selection loading after R2B establishes one tested owner. Do not introduce
   an all-purpose session object.
4. Remove pass-through wrappers that add no validation, caching boundary,
   compatibility value, or meaningful scientific name.
5. Compare repeated validation/report helpers such as planned-run paths, plot
...
6. Remove the `lfp_summary_ppc_runtime` back-import of
   `lfp_summary_runtime` by moving the already shared validation/preparation
   function during R4B.
7. Remove compatibility modules after the R7 audit instead of retaining a
   permanent duplicate navigation surface.
8. Propose retirement of the legacy webapp path-entry branch only after the
...
9. Remove an older PPC executor, CT-specific adapter, profile path, or legacy
...
1. explicit U4 usability acceptance;
2. start of the R0 inventory;
3. start of R1 implementation;
4. start of each later R work package or slice after review of its exact edit
   and tests-first list;
5. deletion of each compatibility or unused module;
6. any real-session numerical run or cache/report mutation;
7. any external copy; and
8. any Slurm submission, monitoring, resume, cancellation, or result
   inspection.
